...
const ensureTempDir = async () => {
  ...
  try {
    ...
    console.error(`[07:15 PM +0530] Failed to create temp directory: ${err.message}`);
...
cmd({
  ...
  try {
    ...
    if (!searchData) {
      ...
      while (retries > 0) {
        ...
        try {
          ...
          console.log(`[07:15 PM +0530] Attempting search API call to ${searchUrl}`);

          const searchResponse = await axios.get(searchUrl, { timeout: 15000 });

          console.log("[07:15 PM +0530] Raw search response:", JSON.stringify(searchResponse.data, null, 2));
          ...
          if (!Array.isArray(searchData) || searchData.length === 0) {
            ...
            console.log("[07:15 PM +0530] Search API returned empty or invalid data after filtering:", searchData);
          ...
          console.log(`[07:15 PM +0530] Successfully fetched ${searchData.length} movies`);
          ...
          console.error(`[07:15 PM +0530] Search API call failed (attempt ${4 - retries}/3): ${error.message}`, error.response?.data || "No response data");
    ...
    films.forEach(film => {
      ...
      console.log(`[07:15 PM +0530] Movie ${film.number}: ${film.title} (Link: ${film.link})`);
    ...
    const filmSelectionHandler = async (update) => {
      ...
      console.log(`[07:15 PM +0530] Selected movie: ${selectedFilm.title} (Link: ${selectedFilm.link})`);
      ...
      try {
        ...
        console.log(`[07:15 PM +0530] Attempting details API call to ${detailsUrl}`);

        const detailsResponse = await axios.get(detailsUrl, { timeout: 15000 });

        console.log("[07:15 PM +0530] Raw details response:", JSON.stringify(detailsResponse.data, null, 2));
        ...
        if (details.imageLinks && details.imageLinks.length > 0) {
          ...
          console.log(`[07:15 PM +0530] Selected thumbnail: ${thumbnailUrl}`);

        }

      } catch (detailsError) {

        console.error(`[07:15 PM +0530] Details error: ${detailsError.message}`);
      ...
      console.log(`[07:15 PM +0530] Attempting download API call to ${downloadUrl}`);
      ...
      while (downloadRetries > 0) {
        ...
        try {
          ...
          console.log("[07:15 PM +0530] Raw download response:", JSON.stringify(downloadResponse.data, null, 2));
          ...
          console.error(`[07:15 PM +0530] Download API call failed (attempt ${4 - downloadRetries}/3): ${error.message}`, error.response?.data || "No response data");
      ...
      const qualitySelectionHandler = async (updateQuality) => {
        ...
        console.log(`[07:15 PM +0530] Selected quality: ${selectedLink.quality} (URL: ${selectedLink.url})`);
        ...
        if (selectedLink.url.includes("pixeldrain.com/u/")) {
          ...
          downloadUrlDark = `https://www.dark-yasiya-api.site/download/pixeldrain?url=${encodeURIComponent(selectedLink.url)}`;

          console.log(`[07:15 PM +0530] Attempting Dark-Yasiya API call to ${downloadUrlDark}`);
          ...
          while (darkRetries > 0) {
            ...
            try {
              ...
              const darkResponse = await axios.get(downloadUrlDark, { timeout: 15000 });

              console.log("[07:15 PM +0530] Raw Dark-Yasiya response:", JSON.stringify(darkResponse.data, null, 2));

              if (darkResponse.data.status && darkResponse.data.result?.dl_link) {

                selectedLink.url = darkResponse.data.result.dl_link;
              ...
              console.error(`[07:15 PM +0530] Dark-Yasiya API call failed (attempt ${4 - darkRetries}/3): ${darkError.message}`, darkError.response?.data || "No response data");
        ...
        try {
          ...
          while (downloadAttempt < maxAttempts) {
            ...
            try {
              ...
              console.error(`[07:15 PM +0530] Download attempt ${downloadAttempt}/${maxAttempts} failed for ${selectedLink.url}: ${error.message}`, error.response?.status, error.response?.headers);
          ...
          console.error(`[07:15 PM +0530] Failed to send movie: ${downloadError.message}`, downloadError.stack);
    ...
    console.error("[07:15 PM +0530] Error in film command:", e);
