...
const ensureTempDir = async () => {
  ...
  try {
    ...
    console.error(`[10:35 AM +0530] Failed to create temp directory: ${err.message}`);
...
cmd({
  ...
  try {
    ...
    console.log(`[10:35 AM +0530] Processing MEGA URL: ${megaUrl}`);
    ...
    console.log(`[10:35 AM +0530] File attributes loaded: ${file.name}, Size: ${file.size} bytes`);
    ...
    while (downloadAttempt < maxAttempts) {
      ...
      stream.on('data', (chunk) => {
        ...
        console.log(`[10:35 AM +0530] Downloaded ${downloadedSize} bytes`);

      });

      stream.on('error', (error) => {

        console.error(`[10:35 AM +0530] Stream error on attempt ${downloadAttempt + 1}: ${error.message}`);
      ...
      if (fileStats.size >= file.size || fileStats.size >= 1024 * 1024) { // At least 1MB or full size
        ...
        console.log(`[10:35 AM +0530] Attempt ${downloadAttempt}/${maxAttempts} failed, size ${fileStats.size} bytes, retrying...`);

        if (downloadAttempt === maxAttempts) {

          throw new Error(`Downloaded file size (${fileStats.size} bytes) does not match expected size (${file.size} bytes) after ${maxAttempts} attempts`);
    ...
    console.log(`[10:35 AM +0530] Successfully sent ${file.name} and cleaned up`);

  } catch (e) {

    console.error("[10:35 AM +0530] Error in mega command:", e);
